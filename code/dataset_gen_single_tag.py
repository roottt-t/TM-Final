...
def tokenize_text(text):
    tokens = []
    offsets = []
    for match in re.finditer(r'\w+|[^\s\w]', text):  # 匹配每个非空格单词
...
def assign_labels(tokens, offsets, annotations):
    ...
    for annotation in annotations:
        for idx, (start, end) in enumerate(offsets):
            ...
            # print("annotation-start-end:",annotation["start"], annotation["end"])
            if start >= annotation["start"] and end <= annotation["end"]:  # 如果 token 在实体范围内
                labels[idx] = f"{annotation['type']}"
            # print("token-label:", tokens[idx], labels[idx])
    return labels
...
if __name__ == "__main__":
    ...
    with open('train_single_tag.txt', 'w') as f:
